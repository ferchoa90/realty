...
class Medico_motivoconsulta extends Component
{
    private const MODULO='MOTIVO CONSULTA';

    public function getMotivoconsultas($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $response= Motivoconsulta::find()->where(["isDeleted"=>0])->all();
        }else{
            $response= Motivoconsulta::find()->where(["isDeleted"=>0])->one();
        }
    }

    public function getMotivoconsulta($id,$condicion=NULL,$itemsret=NULL)
    {
        $result=array();
        if ($id){
            $result= Motivoconsulta::find()->where(["id"=>$id])->one();
            if ($result)
            {
    ...
    public function Editar($data)
    {
        ...
        if ($data):
            //$data = $usuario;
            $dataModel= Motivoconsulta::find()->where(["id"=>$data['id']])->one();
            $dataModel->nombre=$data['nombres'];
            $dataModel->descripcion=$data['descripcion'];
    ...
    public function Eliminar($id)
    {
        ...
        if ($id):
            //$data = $usuario;
            $dataModel= Motivoconsulta::find()->where(["id"=>$id])->one();
